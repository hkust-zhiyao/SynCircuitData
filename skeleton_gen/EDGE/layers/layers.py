...
class MiniAttentionLayer(torch.nn.Module):
    ...
    def forward(self, node_us, node_vs, edges):
        ...
        q_node_us, k_node_us, v_node_us = self.qkv_node(node_us).chunk(3, -1) 
        q_node_vs, k_node_vs, v_node_vs = self.qkv_node(node_vs).chunk(3, -1) 
        q_edges, k_edges, v_edges = self.qkv_edge(edges).chunk(3, -1) 

        q = torch.stack([q_node_us, q_node_vs, q_edges], 1) 
        k = torch.stack([k_node_us, k_node_vs, k_edges], 1) 
        v = torch.stack([v_node_us, v_node_vs, v_edges], 1) 
...
class TGNN_degree_guided(torch.nn.Module):
    ...
    def forward(self, pyg_data, t_node, t_edge):
        ...
        nodes_t = nodes_t[..., None] / self.max_degree  
